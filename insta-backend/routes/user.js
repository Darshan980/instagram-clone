...
// ============================
// PROFILE UPDATE ROUTE - FIXED TO HANDLE ALL FIELDS
// ============================

router.put('/profile', authenticateToken, upload.single('profilePicture'), async (req, res) => {
  try {
    const { fullName, bio, website, phoneNumber, gender } = req.body;
    const userId = req.user._id;

    console.log('=== PROFILE UPDATE REQUEST ===');
    console.log('User ID:', userId);
    console.log('Request body:', { fullName, bio, website, phoneNumber, gender });
    ...
    // Validate and update bio - allow empty string
    if (bio !== undefined) {
    ...
    // Handle website - validate URL if provided
    if (website !== undefined) {
      const trimmedWebsite = website.trim();
      if (trimmedWebsite === '') {
        // Allow clearing the website
        updateData.website = '';
      } else {
        // Validate URL format
        try {
          ...
          return res.status(400).json({
            success: false,
            error: 'Please provide a valid URL for website (e.g., https://example.com)'
          });
        }
      }
    }

    // Handle phone number - allow empty or valid format
    if (phoneNumber !== undefined) {
      const trimmedPhone = phoneNumber.trim();
      updateData.phoneNumber = trimmedPhone;
    }

    // Handle gender - validate against allowed values
    if (gender !== undefined) {
      const validGenders = ['', 'male', 'female', 'other'];
      if (!validGenders.includes(gender)) {
        return res.status(400).json({
          success: false,
          error: 'Invalid gender value. Must be one of: male, female, other, or empty'
        });
      }
